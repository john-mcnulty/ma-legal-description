...
**Extraction mode (v3.27; Plymouth since v3.47).** This user's setting — extraction_mode: `${user_config.extraction_mode}`. Append `--extraction ${user_config.extraction_mode}` to **every ALIS *and* Plymouth invocation** below. (Middlesex South and Suffolk do not extract inline — there the flag is accepted and ignored, and the `--deliver-text-file` path in the `claude-code` bullet is the normal route.)
