...
**Engines (v3.9):** Norfolk and Barnstable now run on a **pure-HTTP engine by default** (requests + BeautifulSoup, no browser) — runs complete in **~10–35 seconds** instead of ~3 minutes. `--engine {auto,http,playwright}` controls this: `auto` (default) uses HTTP and falls back to the old Playwright engine automatically on hard HTTP failure. Plymouth and Middlesex South remain Playwright-only for the registry work. **Since v3.47 (Plymouth) and v3.48 (Suffolk, Middlesex South) all three browser registries run the same inline extraction, report draft and delivery as the ALIS engines, on their downloaded page images** — under the same `--extraction` modes — so a keyed run on ANY supported registry now finishes on its own (measured: the registry script is ~27–31 s; extraction adds one API call). **No registry still needs the `--deliver-text-file` round trip in a keyed run.** The result JSON's `engine` field reports which engine ran.

**Script:** `${CLAUDE_PLUGIN_ROOT}/scripts/legal_desc_fetch.py`

**Extraction mode (v3.27; Plymouth since v3.47; Suffolk and Middlesex South since v3.48).** This user's setting — extraction_mode: `${user_config.extraction_mode}`. Append `--extraction ${user_config.extraction_mode}` to **every invocation below, on every registry** — the flag is now honoured everywhere.

- **`claude-code`** (default; no API key needed) — the script does everything except read the deed pages, then names them in a `READ THE DEED PDFs ...` note (Plymouth: `READ THE DEED PAGE IMAGES ...` — that registry serves page images, not PDFs; the Read tool handles both natively). **Read those pages yourself** with the Read tool to get the legal description, signing date, parties, tenancy and prior-deed reference, then continue at Step 6. `extraction_error` stays null: this is a mode, not a failure. Two things also fall to you: any candidate or grantor hit whose registry abstract carried no address (Read its `sample_file` page-1 PDF to answer which-parcel), and Step 7 delivery — write your transcribed legal description to a UTF-8 `.txt` and re-run the same command with `--deliver-text-file <that path>` (adding nothing else) so the script produces the standard three-form paste-out instead of you assembling it by hand. **When you transcribe by hand, copy the instrument EXACTLY — never correct a typo, an odd mark of punctuation, or archaic spelling.** (A hand transcription in this mode once silently "fixed" a semicolon inside a date to a comma; the v3.47 inline extraction reproduced the semicolon correctly. The record is the record.)
...
python "${CLAUDE_PLUGIN_ROOT}/scripts/legal_desc_fetch.py" \
  ...
  --extraction ${user_config.extraction_mode}
```
`--first` should be uppercase with no spaces between compound name parts (e.g., `JEANMARIE`).

**Plymouth is a single-shot registry since v3.47** — like Norfolk/Barnstable, and (since v3.48) like Suffolk and Middlesex South too. On a successful keyed run the script extracts the deed from its own downloaded page images and writes `legal_description`, the report draft (`report_file`), the three-form `.txt` (`txt_file`) and the clipboard copy itself, so **skip the Read-the-images step and go to Step 6**. Apostrophes in a street name are safe in `--base-name` (`"14 Baker's Lane ..."`) — v3.47 normalises them; before that they caused a false street-mismatch retry.

**Pinning a known deed on Plymouth — `--book N --page P` (v3.50).** When the vesting deed's Book/Page is already known (a title commitment, `registry_profile.json`, a prior run, or a Bk/Pg recited on a municipal lien certificate), pass both flags. The script opens that instrument with the registry's **Book Search** — exact and name-independent — and skips the grantee name search and its town/street retries entirely, which takes every name-index trap out of the path (prefix matching, a misindexed grantee, the 1000-row cap applied before the date sort, a seller with several parcels in one town). The JSON gains **`pinned_by_book_page: true`** and a `PINNED (v3.50)` note. **A pin names the instrument; it does not prove it is the right one**, so three checks still run and each can warn: **`WARNING (pinned): the seller ... is NOT named as a grantee`** (the pin is the wrong instrument, or title is held under a trust/entity/former name — resolve it before relying on the deed); **`WARNING (pinned): the index address ... does not name`** the subject street (or a town mismatch); and the usual conveyance-type guard (`selected_row_is_not_a_deed`) plus the recording-stamp and address checks on the page images. A Bk/Pg with no instrument returns **`deed_not_found`, exit 2** — check the citation and whether the parcel is Registered Land, which has no Book/Page. Both flags are required together (the run refuses a half-specified pin before searching). Registered Land is not pinnable this way on Plymouth.

**`--verify-grantor-hit BOOK/PAGE` on Plymouth (v3.50).** Same contract as the ALIS flag: the hit must appear in the run's own grantor check, and the result lands in **`grantor_hit_verification`** — `status` (`downloaded` / `download_incomplete` / `download_failed` / `not_found` / `not_performed`), index type/date/address, the detail panel's grantors, grantees and consideration, every page image (2000 px) in `files`, the full `extraction`, and `stamp_check`. The notes classify the verified hit against the subject: **`CRITICAL: verified grantor hit ... is at the SUBJECT property`** for a conveyance, a `NOTE` for a non-conveyance at the subject (a mortgage is not a deed-out), a possible-subject WARNING when only the street name matches, and **`WARNING ... no property address could be extracted — the parcel is UNVERIFIED, not a different parcel`**. A stamp that does not name the hit voids the verification with a WARNING. In `claude-code` mode the files are named for you to Read. Plymouth needs BOOK/PAGE (a bare book is refused pre-flight). Combine with `--book/--page` to keep the main deed pinned.
...
16. **Ownership can change with NO deed — and three things used to hide it (v3.39, item 22).** A **death certificate** vests a survivor by operation of law; a **trustee certificate** changes who signs; a **taking** or **decree** can divest or vest. **(v3.52)** A **Certificate of Entry** (`CRTF ENTRY`) starts a foreclosure by entry, and the lender holds title three years later with no deed, so it gets the same OWNERSHIP-RELEVANT note. **(v3.53, Plymouth)** The registry writes the placeholder **`SEEBK`** ("see book"), or `NONE`, in the Town cell of trust instruments; it is now read as *no town*, not as another town, so a post-acquisition trustee certificate stays in `needs_review`, and an ownership-change hit that cannot be located to any parcel gets an **`OWNERSHIP-RELEVANT: ... could NOT be located`** note. Read that instrument before deciding who must sign. v3.38 added `_instrument_significance` to surface exactly these, and on ALIS it could not see them, because **ALIS Land Court indexes a GENERIC document type — `CERTIFICATE`, `AFFIDAVIT`, `DOCUMENT` — and puts the instrument's real nature in the `Desc` cell** ("DEATH OF <name>", "AFFIDAVIT NO DIVORCE"). Significance now reads **type + Desc** (substrings only; the exact-code vocabulary stays keyed on the type, since Desc is staff-typed text that may ESCALATE a hit but never dismiss one). Two more fixes landed with it. **(a) A placeholder address is NOT an address.** The registry writes a literal **`Addr: N/A`** rather than leaving the field blank, and that string was being consumed as a real, non-matching address — silently demoting a row from `parcel unknown` (kept) to `other street, subject town`. `N/A`, `NONE`, `UNKNOWN`, `SEE RECORD` and friends now read as *no address*, which is the v3.20 rule applied one layer out. **(b) On Registered Land the Certificate of Title number is an EXACT parcel key, and it now outranks every address.** A grantor hit whose `Ctf#` equals the selected deed's `Ctf#` is tagged **SUBJECT PROPERTY** with no abstract fetch at all; a *different* certificate is positive evidence of a different parcel; a **missing** certificate on either side asserts nothing and falls through to the address/town logic. **What this means when you read a Land Court run:** the `Ctf#` on each hit line is the parcel answer — trust it over the `Addr:` field, and treat a subject-certificate hit as at the subject parcel even when its address is absent or reads `N/A`. (Live origin: a Norfolk Land Court run where the named seller had **died**, title had passed to his spouse by entirety survivorship with no deed recorded, the deed-out check was correctly clean, and the death certificate was demoted out of `needs_review` by all three defects at once.)

17. **ALIS name-field length cap — over-length names used to fail SILENTLY (v3.43).** The registry's own form hard-caps the name inputs: **`W9SNM`/`W9GNM` = 30 characters (Recorded Land)** and **`W9SN8`/`W9GN8` = 28 characters (Land Court)**. An over-length name was **not rejected** — it degenerated, two different ways depending on town scope, and both produced a wrong answer at exit 0. Measured live 2026-08-24 with a 45-character entity name: a **town-scoped** search returned **ZERO rows**, which the v3.20 cap-retry consumed as "nothing indexed" and reported as **`"0 row(s), COMPLETE ... the subject-parcel check is complete"` — a FALSE CLEAN from a search that structurally could not return a row**; a **`town=*ALL`** search returned **~149 rows whose NAME CELL WAS BLANK**, which passed the conveyance filter and became **phantom grantor hits attributed to the seller** (deeds belonging to unrelated parties in other towns). The same run did both at once: **12 strangers' instruments reported as the seller's, and all 3 genuine subject-parcel instruments missed.** Names are now **clipped to the field limit inside `_alis_url()`**, the one place every ALIS query is built, so no caller can bypass it. **Clipping is safe AND correct — both indexes are PREFIX-matched, so a shorter name returns a strict SUPERSET of the full name's hits.** A `"NOTE: name clipped to the ALIS field limit ..."` note says when it fired, and `grantor_check.searches` / `meta` carry `name_clipped`. **What this means when you read a run:** a clipped search is *wider*, not narrower — **no hit is lost, but same-prefix strangers may appear, so verify the party name on any hit** (the same rule as the surname-only pass). Separately, a result set whose rows are mostly **blank-named** is now refused with `AlisDegenerateResultError` and recorded as an **ERROR / `incomplete_searches`**, never as an empty result — returning `[]` there is indistinguishable from "nothing indexed", which is the false clean itself. **Entity sellers are the exposed case**: the automatic name builders (the exact indexed grantee string, "co-owner from abstract", the surname-only fallback) routinely exceed 30 characters for an entity and rarely do for a person, which is why this stayed latent.

18. **Both registry sections are now searched, and `--office` pins one (v3.44).** The ALIS grantee search ran `for land_court in (False, True)` and **stopped at the first section that returned rows**, so **Land Court was UNREACHABLE for any seller who also owned a Recorded Land parcel** — the Recorded hit won at exit 0 with no warning that a Registered Land parcel existed. Live 2026-08-24: a **Registered Land** subject parcel came back as the seller's **adjoining Recorded Land parcel**, the Kilbride / 29 Fox Meadow failure mode. **`--book` could not rescue it** — the pin was only applied inside the section that had already returned rows, so targeting the Land Court document number failed with `"did not match any result row"` and `deed_not_found`. **Nor could a name prefix**, because the v3.43 28-character Land Court cap truncates a long entity's two indexed spellings to the same string. The search now runs **both sections and selects across the COMBINED candidate set** (what Suffolk has done since v3.40), so `multiple_deed_candidates` can span sections and the wrong-parcel guard compares a Recorded and a Registered candidate side by side. **`result["land_court"]` is now set from the SELECTED ROW**, not from whichever section answered first — which also makes it correct after an auto-retarget that crosses sections. When **both** sections return rows you get a **`CRITICAL: this seller has conveyance rows in BOTH Recorded Land and Registered Land`** note: selection is automatic, but that is the classic wrong-parcel trap, so confirm the selected deed's address or certificate against the subject property. New **`--office {auto,recorded,registered}`** for Norfolk and Barnstable (previously Suffolk-only) pins the index when you already know the section — the Step 1 "Registered Land known" routing rule is now a flag rather than a manual Step 1C detour.

19. **For an ENTITY seller the surname-only pass IS the seller, so its non-conveyance hits are kept (v3.45).** An empty `--first` is this tool's **entity convention** — the whole name goes in `--last` — not an unknown first name. The grantor check's net rule keyed on exactly that (`not first` ⇒ broad deed-out net), so **an entity seller's OWN pass was classified as a stranger-catching net and type-filtered to conveyances**. For a personal surname that filter is right (it is what stops another Sarno's mortgage being reported as the seller's); for an entity it is inverted, because the "surname" is the seller's exact, complete name. Live 2026-08-24: a run returned the correct 3 instruments and skipped **all three** as `rows_skipped_non_conveyance`, so `grantor_check.deeds` came back **empty** and the report showed **none of the seller's encumbrances at the subject parcel** — two mortgages (one an open six-figure) and a municipal lien certificate. The deed-out answer was right; the encumbrance picture was missing. The seller's pair is now marked non-net explicitly, its hits are kept regardless of type, and the redundant surname-only fallback is skipped for an entity (it is the same query, narrowed to the deed group). **Standing limitation, unchanged:** neither pass reaches a **misspelled entity name** in the index — that is the address search's job.

    **Also fixed here (v3.45): a prefetched grantor search from the WRONG SECTION.** The grantor searches prefetch on a worker thread against the deed selected *so far*; since v3.44 the grantee search spans both sections, so the v3.14 auto-retarget can move the selected deed **across sections** afterwards. The prefetch entry recorded its date window but not its **section**, so **Land Court rows were served for a Recorded Land deed** — 1 row instead of 3 — and rendered as `"no subsequent instruments found — clean title"`. The section is now snapshotted once and recorded, and a cross-section prefetch is **discarded and re-searched live** with a note saying so. Same family as the v3.29 stale-window discard, and the same rule as always: **a set that answers a different question is not an empty set.**

20. **A NAME-INDEPENDENT Land Court tripwire on the combined address index (v3.46).** v3.44 searches both sections, but only through the NAME index — so a **misindexed, variant-spelled or successor owner name** still hides a Registered Land parcel, which is the v3.45 standing limitation and the original Kilbride failure. Norfolk's *"Search by Address (Recorded and Registered Land Combined)"* queries **both sections in one request with no name at all**, so it survives all of it. After a **Recorded Land** selection the run now asks that index whether the subject address has a Registered Land deed, and the answer lands in **`land_court_tripwire`** plus a note. **On `CRITICAL (v3.46 Land Court tripwire)` — naming a `Doc`/`Ctf` at the subject address — stop and confirm the section before reporting anything; re-run with `--office registered` if the parcel is Land Court.** Three things bound what a CLEAN tripwire means, and all three are in the note: address coverage is reliable **only from 2003**, the query is restricted to the **deed group**, and a truncated result is reported as truncated. **A clean tripwire SUPPORTS a Recorded Land selection; it never proves one, and `checked: false` means the question was not asked at all** — skipped for an entity seller with no street, for a `--book`/`--page` pin, and when the selected deed is already Land Court. Two mechanics were measured and matter if you ever drive this index by hand: **it paginates** (page 1 shows only ~3 documents, and reading page 1 alone is what once made this index look "sparse and unusable"), and **the suffix spelling decides whether Land Court rows come back at all** — `W9PADR` is prefix-matched, so the tool queries `"<number> <STEM>"` with the suffix dropped, which is a strict superset of both spellings. (An earlier note recorded this as "a house NUMBER suppresses Land Court rows"; that was a page-1-only read, and the suppressed row was on page 2.)
...
**Invocation — Middlesex South (v3.8; inline extraction since v3.48):**
```bash
python "${CLAUDE_PLUGIN_ROOT}/scripts/legal_desc_fetch.py" \
...
- **Single-shot since v3.48:** the script extracts the deed from its own page images and writes `legal_description`, the report draft (`report_file`) and the three-form `.txt` (`txt_file`) itself, with the same recording-stamp and address cross-check notes as Plymouth. On a keyed run skip the Read-the-images step and go to Step 6. This registry is Recorded Land only, so the Land Court stamp/certificate rules below never apply here.
...
- **`timings` (v3.47)** — per-stage seconds (`STEP 1 - grantee search` … `STEP 5 - grantor check`, plus `STEP 6 - inline extraction` when it ran), `total_seconds`, `slowest`. **(v3.54)** On Plymouth, Middlesex South and Suffolk the extraction starts as soon as the page images are saved and runs DURING the grantor check; its stage is then labelled `... (overlapped with grantor check)`, keeps its full `seconds`, and carries a `wall_seconds` — the time the run actually waited for it — which is all that is added to `total_seconds`. When reporting elapsed time, a `wall_seconds` near zero means the extraction was free. Rendered as the report's Run Timings table when `show_timings` is on.
...
- **`selected_row_town_mismatch` (v3.51, Plymouth)** — `true` when the finally-selected instrument is indexed to a **different known Plymouth County town** than the subject. **When `true`, STOP: it is probably another parcel — a same-numbered address in another town.** Do not extract a legal description from it until the deed itself shows the subject town. A matching `"CRITICAL: ..."` note is emitted (on a `--book/--page` pin the existing `WARNING (pinned)` note covers it). A town code the script does not recognise does NOT set the flag — it gets a `NOTE` that the town could not be checked, because an unknown code is a dictionary gap, not evidence of another town. Why it exists: Plymouth's address search is county-wide (v3.42), and until v3.51 every address pick took the highest Book in the county; a Lakeville run returned a Marion deed at exit 0 when the Lakeville code `LKVL` was missing from the dictionary.
...
- **`detail_references` (v3.24; complete since v3.50)** — the detail panel's References cross-ref list (later homesteads, discharges, death certificates, related deeds recorded against the selected deed). Same data as Middlesex South's detail-panel References and the ALIS `abstract.refs`; genuine title signal — surface it in the report's notes and hand it to the discharge workflow. Also echoed as a `"Detail panel references"` note. **Before v3.50 this list was cut off at 10 entries with no warning — on Plymouth, Middlesex South AND Suffolk** (same registry software): the References grid pages 10 rows at a time and the reader took page 1 only (a subdivision covenant's 11th reference — a lot release — and 9 of a master deed's 19 amendments were invisible; a Suffolk master deed kept 10 of 683). The same fields and warning now appear on all three registries. It now walks every pager page and checks the total against the panel's own `References - N` caption: **`detail_references_expected`** is that N and **`detail_references_complete`** says whether all of them were read. **When `detail_references_complete` is `false`, a `WARNING ... the cross-reference list is INCOMPLETE` note says how many were read — never treat a reference missing from a short list as proof the instrument does not exist.** The read stops after 500 references (50 grid pages) and says so; that is reachable on a master deed, essentially never on a vesting deed.
- `notes` — contains raw result row text and detail panel text; read to verify address and extract names. Includes:
  ...
  - `"WARNING: town filter matched 0 of N name-search row(s), and town code(s) [...] are unknown to the script — probably a DICTIONARY GAP"` (v3.51) — the multi-row version of the NOTE above, and the one that matters: an unknown town code on a multi-row search is what sent a Lakeville run to a Marion deed. Add the code to `_PLYMOUTH_TOWN_ABBREVS` if it is the subject town
  - `"<label>: scoped to town '...' — kept N of M row(s)"` / `"WARNING: <label>: no row is indexed to town '...', but N row(s) at this address are ALSO in the seller's own grantee search"` / `"... using the N row(s) with NO town indexed ... UNVERIFIED"` / `"CRITICAL: <label>: EVERY row is indexed to a town other than '...'"` (v3.51) — how an address-search pick was scoped to the subject town; see `selected_row_town_mismatch`
  ...
  - `"CRITICAL: the selected instrument Bk.../... is indexed to town '...', NOT '...'"` (v3.51) — see `selected_row_town_mismatch` above
...
4. **Town-mismatch address-search retry.** If the name search selects a deed whose town doesn't match the expected town AND street info is available (auto-parsed from `--base-name`), the script automatically retries using the property address search — without requiring a second manual invocation. **(v3.51) The address search is county-wide, so every address pick — this retry, the v3.3 misindexed-name fallback, `--force-address-search` and the trust/LLC fallback — is now scoped to the subject town before a row is chosen:** rows indexed to the subject town first; else rows the seller's own grantee search also returned (this is what still finds the right deed when a town code is missing from the dictionary); else rows with no town indexed, reported as UNVERIFIED; else a `CRITICAL` note. Before v3.51 the pick was the highest Book in the county, so a same-numbered address in another town could win.
...
7. **(v2.9) Single-result mismatch no longer triggers retry.** If the name search returns exactly one result and its town abbreviation isn't in the dict, the script accepts the row and logs a `NOTE` rather than firing the address-search retry. The retry is now gated on `len(all_rows) > 1` — it only fires when there is genuine multi-property ambiguity. This eliminates false retries caused by unknown-but-correct abbreviations (e.g. `PLMTH` before it was added to the dict). **The gate does not make the dictionary optional:** on a multi-row search an unknown code means the town filter matches nothing, and before v3.51 that retry could cross town lines (a Lakeville run returned a Marion deed at exit 0). v3.51 adds `LKVL`/`MRION`, names the gap in a multi-row `WARNING`, scopes the retry to the subject town and the seller's own rows, and flags a wrong-town selection with `selected_row_town_mismatch`.
...
**Suffolk & Middlesex South v3.48 (item 42) — inline extraction, and the two things it changes about what you REPORT.** Both registries now extract the deed from their own page images, write the report draft and deliver the three paste forms, exactly as Plymouth has since v3.47. On a keyed run, **skip the Read-the-images step and go to Step 6**; in `claude-code` mode the old `READ THE DEED PAGE IMAGES ...` note and the `--deliver-text-file` route are unchanged. Two verification notes come with it, and on Registered Land they are not the Recorded Land ones:

- **The recording-stamp check is three-way, and Land Court is keyed on the DOCUMENT NUMBER.** Registered Land has no Book/Page, so a Land Court stamp is checked against Document No., corroborated by "Noted on Certificate" and the Land Court registration book/page. `"Recording stamp verified: ..."` means the images name the selected instrument. **`WARNING: the recording stamp ... does NOT name the selected ...` means the viewer served a different instrument — stop.** The third outcome is new and is neither: **`NOTE: a recording stamp was read ... but it could NOT be checked`** means the check did not run (an older paper filing with no cover sheet, or no identifier on both sides). **That is an unchecked box, not a clean check** — confirm the images against the index row yourself. This tier exists because the v3.47 check keyed on Book/Page alone would have warned on every *correct* Land Court run, and a warning that fires every time is worse than none.
- **The Certificate of Title is authoritative only from the cover sheet or the detail panel — never from a certificate recited in the deed.** A certificate recited in the body or the property description is the certificate the land is **DESCRIBED on** (the one being conveyed *out of*); the one to cite is the certificate this deed is **noted on**. The run resolves that itself and says which source it used: `"Certificate of Title NNNNN read off the cover sheet's 'Noted on Certificate' line"` (the panel returned nothing on that run), `"Certificate corrected to NNNNN: the extraction returned MMMMM ..."`, or — the one to act on — **`WARNING: certificate disagreement`** (panel and cover sheet name different certificates) and **`WARNING: Certificate of Title NNNNN came from the deed text alone`** (**do NOT cite it until confirmed on the images**). Live origin 2026-09-02: the panel's Certificate/Encumbrance read came back empty, extraction filled 77105 off the instrument, and the cover sheet read "Noted on Certificate : 198332".
...
### Step 5 — Capture the Source Pages

Save every page the legal description was taken from, to the output folder. See registry-specific technical notes below for capture methods.

**The test is auditability, not file format.** Any capture — the registry's PDF, a hi-res render, a viewer screenshot — is acceptable provided a human can open it and visually confirm that the captured legal description matches the recorded instrument. That means: one file per page, openable without special tooling, **legible at the size saved**, and carrying its recording citation. A page saved at a viewer's thumbnail resolution fails this test even when it happens to be readable; the fast paths fetch a 2000 px render and say so per page (`hires_src_download`), warning when any page came back smaller.
...
**Fast-path shortcut (v3.16, Norfolk/Barnstable HTTP engine; Plymouth since v3.47):** if the script JSON contains `report_file`, the report draft already exists — Read it, review/adjust the Title Flags section (items tagged "review" are judgment calls; verify grantor-hit assessments against the `via:` guidance above), remove the DRAFT banner, and save. Do not write a new file from scratch.
...
- **Typical elapsed time (Playwright fast path):** **~30 s of script on Plymouth** (v3.47 measurement: 31.5 s for grantee search + 3-page pager walk + detail panel + 3 image downloads + 45-row grantor check; inline extraction adds one API call, and the old 3–5 minute figure was the assistant transcribing the images by hand after a script that had already finished); 3–5 minutes on Norfolk and Barnstable via Playwright (the HTTP engine is 10–65 s); **~16–31 s of script on Suffolk** (v3.41 measured 16 s on a seller with rows in one office; a seller searched in both offices with 5 pages to download runs ~27–31 s. Was ~70 s at v3.40 — the difference is almost entirely the empty-search timeout, not the searches themselves). **Since v3.48, Suffolk and Middlesex South add inline extraction like Plymouth's — budget ~55–65 s end to end on either, single-shot.** Manual fallback paths run 15–30 minutes on well-behaved registries. The old 45–70+ minute Suffolk figure described the manual browser path and no longer applies to `--registry suffolk`.
...
|---|---|---|---|
| Plymouth County | 10 | ~20 min (manual); ~35 min (v2.7 two-pass); ~13 min (v2.8 two-pass); **~50 s single-shot (v3.47)** | v2.7 two-pass (155R Seabright Rd, Scituate): wrong deed selected, script updated mid-run. v2.8 two-pass (192 Silver Birch Dr, Halifax): HLFX abbreviation bug triggered false mismatch on first run; re-run with `--town HLFX` workaround; script fixed to v2.8 resolves automatically. **v3.47 (measured over 4 live runs on one condo-unit seller): 44–60 s total — registry ~28–31 s (grantee search, 3-page pager walk, detail panel, 3 page images, 45-row grantor check) + inline extraction 17–25 s (Opus, 3 hi-res images). The prior "3–5 min single-pass" figure was NOT the script: it was the assistant reading the page images and transcribing by hand after a script that had already finished. `--deliver-text-file` re-entry, when needed, is 2.7 s and touches no registry.** |
...
| Suffolk County | 7 | ~66 min (run 1, manual); **~60-70 s (v3.40, run 2)**; **~16 s (v3.41, run 3)**; **~55-64 s single-shot (v3.48, runs 4-7)** | Run 2 (15 Larkspur Road, Boston — Hollister): first live `--registry suffolk`, **Registered Land**, exit 0, Doc 812445 noted on Ctf 198332, 3 hi-res pages; grantor check surfaced the same-day purchase-money mortgage (Doc 812446) and homestead (Doc 812447). This run also caught three defects: a prefix-matched first name (`JULIAN`→`HOLLISTER JULIANA`) selecting another party's parcel, a silent Office-switch race returning Recorded Land rows labelled as Land Court, and the town/date filters suppressing rows. Run 3 (Recorded Land condo unit, 2026-08-20): v3.41 — the combined `Party Type = Both` search halved the registry searches (4 to 2) and the grantor check consumed the `GR` rows with no extra query, but the real cost was elsewhere: an EMPTY search sat out the full 45 s grid timeout, so a seller with no Land Court records paid ~28 s per search averaged over four. Consuming the registry's own "0 hits" dialog took the run from ~70 s to ~16 s with identical findings. The same probe also found the 1,000-record cap returning zero rows and being read as "nothing indexed". **Runs 4-7 (v3.48, 2026-09-02): inline extraction ported here — Recorded (Vandermeer / 22 Cardiff St Unit 1, Bk 59214/117, 5 pages, 64 s) and Registered (Hollister / 15 Larkspur Rd, Doc 812445 / Ctf 198332, 3 pages, ~55 s x3). Script ~27-31 s + extraction ~20-30 s. Fidelity vs the verified hand transcriptions: the Land Court text was byte-identical and the Recorded text matched 320/320 words, the only delta being the derivation clause, which the extraction correctly leaves to `prior_deed_reference`. These runs found the false Land Court stamp warning and the wrong-certificate fill — see v3.48 above.** |
| Middlesex South | 2 | **3.3 min (v3.8 fast path, run 1)**; **54 s single-shot (v3.48, run 2)** | Run 1 (531 Fieldstone Brook, Acton — Ostrander): exit 0 first try, 3 hi-res pages, detail-panel References surfaced a 2022 death cert + 2024 complaint automatically. Headful Chrome (Incapsula) worked unattended. **Run 2 (7 Ashcombe Rd Unit 1, Medford — Draycott, Bk 70921/188, 2026-09-02): first v3.48 inline-extraction run here — 54 s end to end, stamp and address both verified, report draft and three-form .txt written by the script.** |
