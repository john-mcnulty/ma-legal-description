...
| `extraction_mode` | `claude-code` (default) reads the deed PDF in this session — no API key, no extra cost. `api` calls the Anthropic API for a faster single-shot run and needs `ANTHROPIC_API_KEY` in the environment. |
...
| Suffolk County | Boston, Chelsea, Revere, Winthrop | https://www.masslandrecords.com/suffolk/D/Default.aspx | Avenu/20-20 (ASP.NET) | 🔶 Ready (new) |
...
- **`selected_row_is_not_a_deed` (v3.12)** — `true` when the finally-selected instrument is NOT a conveyance deed (e.g. an ASSIGNMENT or MORTGAGE), meaning no DEED-type row for the property was found in either the grantee name index or the address index. **When `true`, STOP: do not extract a legal description and do not report the instrument as the vesting deed.** Check Plymouth Registered Land (Land Court) and check for a misindexed grantee name. A matching `"CRITICAL: ..."` note is also emitted.
...
- **`grantor_check.needs_review` (v3.21)** — the short set of grantor hits that actually need judgment, pulled out of a list that runs to the dozens on a common name. A hit is here when it is at (or cannot be excluded from) the subject parcel, **or** when it is a conveyance-type instrument recorded on/after the acquisition date. **Read this first.** `grantor_check.deeds` still contains every hit — nothing is dropped — now ordered most-relevant first with a parcel tag appended to each line.
...
3. **Cover sheet or header** — Suffolk County deeds have a recording cover sheet (page 1) that may include the address; some registries stamp the address at the top or bottom of the first page.
...
- **Typical elapsed time (Playwright fast path):** 3–5 minutes on Plymouth, Norfolk, and Barnstable. Manual fallback paths run 15–30 minutes on well-behaved registries. Suffolk County runs may take 45–70+ minutes due to renderer freeze issues.
...
- **Browser tool calls (token proxy):** Target under 50 per run on ✅ Ready registries. First runs on new registries and Suffolk County runs with renderer freezes will exceed this. Playwright runs consume ~0–5 browser tool calls total (the Playwright script itself does not count — it's invoked via Bash) vs. 30–60 for the full manual path.
- **Image resolution (Suffolk):** The ~217×281 px native images are a known limitation of masslandrecords.com. If a clearer image is needed, retrieve the deed directly at the Registry or via the Registry's pay-per-page download service.
...
| Suffolk County | 1 | ~66 min | Renderer freezes, popup blocker issues on first run |
